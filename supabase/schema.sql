...
create table production_requests (
  ...
  automation_steps jsonb not null default '[]'::jsonb,
  ...
  approval_options jsonb not null default '[]'::jsonb,
  approval_status text not null default 'none',
  approval_answer jsonb,
  extra_credit_required integer not null default 0,
  legal_acceptance_id uuid,
  legal_acceptance_snapshot jsonb not null default '{}'::jsonb,
  estimated_credits integer not null default 0,
  reserved_credits integer not null default 0,
...
create table production_shot_jobs (
...
create index production_shot_jobs_production_idx on production_shot_jobs (production_id, shot_index);
...
create table live_sales_agents (
...
create index live_sales_agents_user_updated_idx on live_sales_agents (user_id, updated_at desc);
...
create policy "live sales agents own read" on live_sales_agents for select using (auth.uid() = user_id);
create policy "live sales agents own insert" on live_sales_agents for insert with check (auth.uid() = user_id);
create policy "live sales agents own update" on live_sales_agents for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
