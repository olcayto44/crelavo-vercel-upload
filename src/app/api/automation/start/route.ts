...
function buildHeyGenVideoAgentPrompt(input: { title: string; prompt: string; script?: string; durationSeconds?: number; aspect?: string; hasVisualFiles?: boolean; providerPrompt?: string }) {
  ...
  return [
    ...
    "Subtitle safe-area guard: include readable captions only inside the mobile-safe lower-third area. Keep subtitle text away from the bottom edge and never let captions spill into black bars or outside the vertical 9:16 safe area.",
    ...
    `Duration lock: target duration is ${duration} seconds. Treat this as a strict target; for a 30-second request aim for 28-32 seconds and do not expand into a 40+ second video unless the user explicitly approved it.`,
    "Audio direction: include confident voiceover/dialogue, energetic background music under the voice, and a polished social-ad mix where the voice is always clear. The presenter must sound bright, awake, confident, and energetic; avoid sleepy, flat, bored, breathy, low-energy delivery.",
