...
export async function POST(req: NextRequest) {
  ...
  if(result.status==="failed" && !taskId.startsWith("runway:") && production?.prompt && runwayConfigured()){
    ...
    if(!alreadyAttempted){
      ...
      if(fallback.ok&&fallback.taskId){taskId=String(fallback.taskId);result={status:"queued",provider:"runway",taskId,message:"MiniMax failed; Runway fallback started."};await supabase.from("production_requests").update({generation_status:"queued",status:"in_production",provider_job_id:taskId,output_json:{...(production.output_json||{}),assistant:true,provider:"runway",taskId,runwayFallbackAttempted:true,fallbackFrom:"minimax"},error_message:null,updated_at:new Date().toISOString()}).eq("id",productionId).eq("user_id",auth.data.user.id);}
  ...
  if(result.status==="failed" && taskId.startsWith("runway:") && production?.prompt && falConfigured()){
    ...
    if(!alreadyAttempted){const fallback=await startFalFallback({prompt:String(production.prompt),category,extras:(production.input_json?.extras && typeof production.input_json.extras === "object" ? production.input_json.extras : {})});if(fallback.ok&&fallback.taskId){taskId=String(fallback.taskId);result={status:"queued",provider:"fal",taskId,message:"Runway failed; FAL fallback started."};await supabase.from("production_requests").update({generation_status:"queued",status:"in_production",provider_job_id:taskId,output_json:{...(production.output_json||{}),assistant:true,provider:"fal",taskId,falFallbackAttempted:true,fallbackFrom:"runway"},error_message:null,updated_at:new Date().toISOString()}).eq("id",productionId).eq("user_id",auth.data.user.id);}else result={...result,message:(result.message||"Runway failed.")+" FAL fallback could not start: "+(fallback.message||"unknown error")};}
  }
  ...
  if(productionId){ await supabase.from("production_requests").update({ generation_status: "ready", status: "ready", provider_job_id: taskId, preview_url: mediaUrl, output_json: { ...(production?.output_json||{}), assistant: true, provider: result.provider || production?.output_json?.provider || "heygen_or_minimax", taskId, audioStatus, audioMuxed: audioStatus === "muxed", media: { kind: "video", url: mediaUrl } }, completed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id",productionId).eq("user_id",auth.data.user.id); }
