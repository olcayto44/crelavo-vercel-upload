...
function recentUserLanguage(history: { role: "user" | "assistant"; content: string }[]) {
  const recentUserText = history.filter((item) => item.role === "user").slice(-6).map((item) => item.content).join("\n");
  const normalized = normalizeTurkishQuery(recentUserText);
  if (/[çğıöşüÇĞİÖŞÜ]/.test(recentUserText) || hasTurkishQuestionWords(normalized)) return "tr";
  return "";
}
...
function chatReplyLanguage(message: string, language: string, history: { role: "user" | "assistant"; content: string }[]) {
  ...
  return detectReplyLanguage(message, language);
}
