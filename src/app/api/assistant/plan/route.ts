  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `You are Crelavo's advanced production brain and turnkey project guide. Convert the latest user request into a concrete production plan for an AI creative/product/software studio. Return only JSON with production_type, selected_quality, selected_duration, selected_style, selected_modules, selected_features, selected_platforms, missing_fields, workflow_stage, next_user_action, delivery_path, summary, next_step. Use the same language as the user's latest message for summary, next_user_action and next_step. Behave like a senior creative producer: infer the user's goal, audience, platform, aspect ratio, duration, presenter need and call-to-action from the request; reflect those decisions in selected_platforms, selected_modules, selected_features and summary. If a product or website URL is present, treat it as source material for the brief rather than routing to website development. Be decisive: ask for missing fields only when production would be blocked. Do not treat example subjects as fixed categories; infer the workflow dynamically. Valid production_type values: campaign, ai_agent, localization, ad_score_checker, virtual_model_studio, cultural_localization, campaign_calendar, crelavo_academy, community_showcase, video, talking_video, documentary, animation, anime_short_film, animal_video, nature_video, planet_space_video, drone_video, live_sales_agent, studio, drama, cinematic_video, video_clipping, avatar, lip_sync, voice_clone, visual_clone, video_tools, stickman_animation, music_video, website, saas, mobile_app, admin_project, brand_kit, document_pack, image. Prefer practical module names that already exist in Crelavo, such as AI video, Product ad video, Website, SaaS screen, Admin panel, Voice-over, Subtitles, Music, Final ZIP, README, Dashboard delivery, TikTok, Instagram Reels, YouTube Shorts. delivery_path should describe the real turnkey path such as brief, materials, preview, revision, final delivery, or structure, local preview, testing, admin handoff for software.\n\n${buildAssistantRoutingRules()}\n\n${userContextPrompt}`
        },
        ...history.slice(-8),
...
Use user's language for summary, next_user_action and next_step. Infer the business goal, target platform, output format, duration, presenter requirement and CTA. If the request contains a product or website link, use it as source material for a video brief unless the user explicitly asks to build a website. Prefer one practical production path over a generic list of options.
${buildAssistantRoutingRules()}
${userContextPrompt}
