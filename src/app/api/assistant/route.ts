...
function normalizeSuggestion(raw: Partial<AssistantSuggestion>, idea: string, mode: AssistantMode): AssistantSuggestion {
  ...
  return {
    ...
    assistantReply: raw.assistantReply?.trim() || (language === "Turkish"
      ? `Fikrini analiz ettim ve yönetimli üretim akışı için en doğru adımı hazırladım: ${category}, ${style}, ${duration}. ${actionRoute.nextStep}.`
      : `I analyzed your request and prepared the best managed production next step: ${category}, ${style}, ${duration}. ${actionRoute.nextStep}.`),
