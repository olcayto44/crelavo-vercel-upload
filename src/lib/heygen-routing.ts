...
export function hasMinimaxPresenterIntent(routeSignal: string) {
  const sanitized = sanitizeProviderRouteSignal(routeSignal);
  return /ai\s+presenter|with\s+presenter|auto\s+choose\s+best\s+presenter|female\s+presenter|male\s+presenter|young\s+energetic\s+creator|energetic\s+ugc\s+creator|video\s+agent\s+auto\s+edit|adult\s+neutral\s+voice|ugc-style\s+product\s+script|minimax_video_agent|minimax|video_agent|video\s+agent|talking\s*avatar|talking\s*head|ugc|koc|creator|product\s*demo|social\s*media\s*ad/.test(sanitized)
    && !/\b(no\s*presenter|without\s*presenter|b-?roll\s*only|silent\s*\/\s*music\s*only|no\s*voice)\b/.test(sanitized);
}

export function shouldForceMinimaxPresenterProvider(input: {
  ...
  return hasMinimaxPresenterIntent(input.routeSignal) && !hasCinematicActionIntent(input.routeSignal);
}
