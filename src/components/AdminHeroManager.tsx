...
export function AdminHeroManager({ initialVideos }: { initialVideos: ConfiguredShowcaseVideo[] }) {
  ...
  function toggleHero(id: string, enabled: boolean) { update(id, enabled ? { heroEnabled: true, publishStatus: "published" } : { heroEnabled: false }); }
  function clearHero() { setVideos((items) => items.map((item) => ({ ...item, heroEnabled: false }))); setMessage("Tüm Hero seçimleri kaldırıldı. Değişikliği uygulamak için kaydet."); }
