...
const firstFrameVideoIds = new Set([
  "ad-creative-angles-showcase",
  "crelavo-wow-reel",
  "crelavo-energy-system",
  "crelavo-product-story",
  "phoenix-awakening",
  "origami-dragon-meteor",
  "turkish-avatar-hook"
]);
...
function categoryFor(video: ConfiguredShowcaseVideo): Exclude<Filter, "all"> {
  ...
  if (/product|ad-creative|lower-ad|serum|skincare|ramen|soda|lip-balm|headphone|summer-face-mist|holiday-skincare/.test(id)) return "product";
  return "cinematic";
}

const css = `
.public-side-rail,.ad-rail,.ad-rail-right,.trial-fomo-rail,.trial-fomo-flash{display:none!important}.page-with-rails{padding-left:0!important;padding-right:0!important}header.site-main-nav{background:#070b18}@media(max-width:980px){header.site-main-nav{display:flex!important;flex-wrap:nowrap!important;align-items:center!important;justify-content:space-between!important;gap:8px!important;min-height:56px;padding:8px 12px!important;position:sticky;top:0;z-index:80;background:#070b18}header.site-main-nav .logo{flex:0 0 auto;z-index:2}header.site-main-nav .nav-links,header.site-main-nav .primary-nav-links{display:none!important}header.site-main-nav .nav-links.is-open,header.site-main-nav .primary-nav-links.is-open{display:flex!important;flex-direction:column;position:absolute;left:12px;right:12px;top:56px;background:#070b18;border:1px solid rgba(255,255,255,.12);border-radius:16px;padding:10px;max-height:70vh;overflow:auto;z-index:90;gap:2px}header.site-main-nav .nav-links.is-open a,header.site-main-nav .primary-nav-links.is-open a{padding:10px 12px;border-radius:10px}header.site-main-nav .tools-mega-menu{display:none!important}header.site-main-nav .nav-session-bar{display:flex!important;align-items:center;gap:6px;flex:0 1 auto;margin-left:auto;max-width:52%;overflow:hidden;flex-wrap:nowrap}header.site-main-nav .header-language-links{display:flex;gap:6px;font-size:11px;flex-shrink:0}header.site-main-nav .nav-session-bar a{font-size:12px;white-space:nowrap}.cl-mnav{display:inline-flex;align-items:center;justify-content:center;margin-left:8px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);color:#f8fbff;border-radius:999px;padding:8px 12px;font-size:13px;font-weight:650;font-family:Inter,system-ui,sans-serif;cursor:pointer}}@media(min-width:981px){.cl-mnav{display:none!important}}@media(max-width:420px){header.site-main-nav .header-language-links{display:none!important}header.site-main-nav .nav-session-bar{max-width:64%}}
#cle{--navy:#070b18;--cyan:#0ea5e9;--muted:#aeb8cc;--card:rgba(255,255,255,.06);--line:rgba(255,255,255,.1);--text:#f8fbff;font-family:Inter,system-ui,sans-serif;color:var(--text);background:var(--navy);padding:28px 20px 80px}#cle *{box-sizing:border-box}#cle a{color:inherit;text-decoration:none}#cle .wrap{max-width:1180px;margin:0 auto}#cle .back{display:inline-flex;border:1px solid var(--line);background:rgba(255,255,255,.06);border-radius:999px;padding:8px 14px;font-size:13px;font-weight:650;margin-bottom:22px}#cle .kicker{display:inline-flex;border:1px solid var(--line);background:rgba(14,165,233,.12);color:#7dd3fc;border-radius:999px;padding:6px 12px;font-size:11px;font-weight:700;letter-spacing:.04em;text-transform:uppercase}#cle h1,#cle h2{margin:0;letter-spacing:-.03em}#cle p{margin:0;color:var(--muted);line-height:1.6}#cle .hero{display:grid;grid-template-columns:1.05fr .95fr;gap:22px;align-items:stretch;margin-bottom:22px}#cle .intro{border:1px solid var(--line);background:linear-gradient(180deg,rgba(14,165,233,.16),rgba(8,16,36,.78));border-radius:28px;padding:28px;display:flex;flex-direction:column;gap:14px}#cle .intro h1{font-size:clamp(36px,5vw,58px);line-height:1.02}#cle .intro .lede{font-size:16px;max-width:520px}#cle .cta-row,#cle .filters{display:flex;flex-wrap:wrap;gap:10px}#cle .btn{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;padding:12px 18px;font-size:14px;font-weight:650}#cle .btn-cyan{background:linear-gradient(180deg,#38bdf8,#0284c7);color:#04203a}#cle .btn-ghost{background:rgba(255,255,255,.06);border:1px solid var(--line)}#cle .hero-art{border:1px solid var(--line);border-radius:24px;overflow:hidden;background:#020617;min-height:280px}#cle .hero-art img{width:100%;height:100%;object-fit:cover;display:block;aspect-ratio:16/9}#cle .cats{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:8px 0 28px}#cle .cat{border:1px solid var(--line);background:var(--card);border-radius:20px;overflow:hidden}#cle .cat img{width:100%;aspect-ratio:16/9;object-fit:cover;display:block}#cle .cat span{display:block;padding:12px 14px 16px;font-size:14px;font-weight:650}#cle .filters{margin:0 0 16px}#cle .filters button{border:1px solid var(--line);background:rgba(255,255,255,.05);color:#dbeafe;border-radius:999px;padding:8px 12px;font-size:13px;font-weight:650;cursor:pointer;font-family:inherit}#cle .filters button.on{background:rgba(14,165,233,.18);border-color:rgba(14,165,233,.45);color:#7dd3fc}#cle .grid{display:grid;grid-template-columns:repeat(3,1fr);gap:18px}#cle .tile{border:1px solid var(--line);background:var(--card);border-radius:22px;overflow:hidden;display:flex;flex-direction:column}#cle .tile .stage{position:relative;aspect-ratio:9/16;background:#020617;overflow:hidden}#cle .tile video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center center;display:block;background:#020617}#cle .meta{display:flex;flex-direction:column;gap:8px;padding:14px 16px 18px}#cle .meta strong{font-size:18px;letter-spacing:-.02em}#cle .meta em{font-style:normal;color:var(--muted);font-size:13px;line-height:1.5}#cle .tile[hidden]{display:none}#cle .band{margin-top:28px;border:1px solid var(--line);background:linear-gradient(180deg,rgba(14,165,233,.14),rgba(8,16,36,.7));border-radius:24px;padding:28px;text-align:center}#cle .band h2{font-size:28px;margin-bottom:8px}#cle .band p{margin-bottom:16px}@media(max-width:980px){#cle .hero,#cle .cats,#cle .grid{grid-template-columns:1fr 1fr}}@media(max-width:640px){#cle .hero,#cle .cats{grid-template-columns:1fr}#cle .grid{grid-template-columns:1fr;max-width:420px;margin:0 auto}}
`;
...
export function ExploreSamplesBody({ videos }: { videos: ConfiguredShowcaseVideo[] }) {
  ...
  return <><InnerMobileNav /><style dangerouslySetInnerHTML={{ __html: css }} /><section id="cle"><div className="wrap">
    ...
    <div className="grid" id="cle-grid">{visible.map((video) => <a className="tile" data-cat={categoryFor(video)} href={`/showcase/videos/${video.id}`} key={video.id}><span className="stage"><video muted loop playsInline preload={firstFrameVideoIds.has(video.id) ? "auto" : "metadata"} {...(firstFrameVideoIds.has(video.id) ? {} : { poster: video.imageUrl || fallbackPoster })} src={video.videoUrl}></video></span><span className="meta"><span className="kicker">{video.kicker}</span><strong>{video.title}</strong><em>{video.description}</em></span></a>)}</div>
    <div className="band"><h2>24 hours to see the studio. Then $9.99/month unless you cancel.</h2><p>Card required. No charge until the preview ends. Annual checkout is temporarily unavailable while its Polar product is prepared.</p><div className="cta-row" style={{justifyContent:"center"}}><a className="btn btn-cyan" href="/pricing#clp">Start 24-hour Pro preview — $9.99/mo</a><a className="btn btn-ghost" href="/pricing">Annual Pro — $99/yr</a></div></div>
  </div></section></>;
}
