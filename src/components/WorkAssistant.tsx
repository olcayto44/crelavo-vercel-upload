...
function defaultSetupFor(type: string, hint = "", plan?: StudioPlan | null): ProductionSetupState {
  ...
  const explicitPresenterIntent = /\b(?:ai\s+presenter|female\s+presenter|male\s+presenter|with\s+(?:an?\s+)?presenter|use\s+(?:an?\s+)?presenter|include\s+(?:an?\s+)?presenter|talking\s+(?:head|host)|spokesperson|sunucu(?:lu|yla))\b/i.test(text);
