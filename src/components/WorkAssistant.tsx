...
const minimaxMusicVibes = [
  ...
  { label: "Calm educational", query: "calm acoustic educational" },
  { label: "UGC social media", query: "trendy social media upbeat" }
];
...
const setupProfiles: Record<string, SetupProfile> = {
  video: {
    ...
    groups: [
      ...
      { id: "presenterChoice", title: "Presenter selection", options: ["No presenter / B-roll only", "Auto choose best presenter", "Female presenter", "Male presenter", "Young energetic creator", "Professional business presenter", "Energetic UGC creator", "Mature trustworthy presenter"] },
      { id: "presenterMotion", title: "Presenter motion", multi: true, options: ["No presenter motions", "Natural delivery", "Smile", "Wave", "Point at camera", "CTA hand gesture", "Energetic gestures"], credit: HEYGEN_MOTION_PROMPT_CREDITS },
  ...
  video_clipping: {
    ...
    groups: [
      ...
      { id: "presenterChoice", title: "Presenter selection", options: ["No presenter / B-roll only", "Auto choose best presenter", "Female presenter", "Male presenter", "Young energetic creator", "Professional business presenter", "Energetic UGC creator", "Mature trustworthy presenter", "AI avatar host"] },
      { id: "quality", title: "Quality", options: sharedVideoQuality, credit: 900 },
...
const trUiLabels: Record<string, string> = {
  ...
  "Ready": "Ready",
  "Page": "Page",
  "Open production": "Open production",
  "Start Production": "Start production",
  "Creating...": "Creating...",
  "AI video setup": "AI video settings",
  "Only video-specific production choices are shown here.": "Choose the video settings needed for this category.",
  "Video style": "Video style",
  "Presenter selection": "Presenter selection",
  "Presenter motion": "Presenter motion",
  "Natural delivery": "Natural delivery",
  "Smile": "Smile",
  "Wave": "Wave",
  "Point at camera": "Point at camera",
  "CTA hand gesture": "CTA hand gesture",
  "Energetic gestures": "Energetic gestures",
  "Auto choose best presenter": "Auto choose best presenter",
  "Female presenter": "Female presenter",
  "Male presenter": "Male presenter",
  "Young energetic creator": "Young energetic creator",
  "Professional business presenter": "Professional business presenter",
  "Energetic UGC creator": "Energetic UGC creator",
  "Mature trustworthy presenter": "Mature trustworthy presenter",
  "AI presenter": "AI presenter",
  "Voice-over only": "Voice-over only",
  "Silent / music only": "Silent / music only",
  "Video type": "Video type",
  "Quality": "Quality",
  "Duration": "Duration",
  "Format": "Format",
  "Source / scene handling": "Source / scene handling",
  "Background / environment": "Background / environment",
  "Pace / transitions": "Pace / transitions",
  "Voice-over": "Voice-over",
  "Extras": "Extras",
  "Single": "Single",
  "Multiple": "Multiple",
  "credits each": "credits",
  "Prompt-to-video": "Prompt-to-video",
  ...
  "Cinematic promo": "Cinematic promo",
  ...
  if (plan.production_type === "video" && isPresenterUgcLock) {
    return {
      ...
      groups: [
        ...
        { id: "heygenQuality", title: "Minimax H3", options: minimaxH3Options },
        { id: "presenterChoice", title: "Presenter selection", options: ["No presenter / B-roll only", "Auto choose best presenter", "Female presenter", "Male presenter", "Young energetic creator", "Professional business presenter", "Energetic UGC creator", "Mature trustworthy presenter"] },
        { id: "presenterMotion", title: "Presenter motion", multi: true, options: minimaxMotionPromptOptions, credit: HEYGEN_MOTION_PROMPT_CREDITS },
...
function defaultSetupFor(type: string, hint = "", plan?: StudioPlan | null): ProductionSetupState {
  ...
  const wantsSubtitles = !noSubtitles && (subtitlesRequestedByPrompt(text) || /subtitles?|altyaz[ıi]/.test(text));
...
const presenterIntentText = text.replace(/\b(?:do\s+not|don't|no|without|not)\s+(?:include\s+)?(?:an?\s+)?(?:ai\s+)?presenter(?:s)?(?:\s+video)?\b/gi, "");
const explicitPresenterIntent = /\b(?:ai\s+presenter|female\s+presenter|male\s+presenter|with\s+(?:an?\s+)?presenter|use\s+(?:an?\s+)?presenter|include\s+(?:an?\s+)?presenter|talking\s+(?:head|host)|spokesperson|sunucu(?:lu|yla))\b/i.test(presenterIntentText);
  ...
  selected.push("Minimax H3");
    ...
    if (group.id === "presenterChoice") {
      if (wantsNoPresenterIntent || narrativeNoPresenter) {
        ...
        const female = hasFemaleCharacterIntent || /female\s*(presenter|creator|host|avatar)|woman\s*(presenter|creator|host|avatar)|kad[ıi]n\s*(sunucu|avatar)/.test(text) ? group.options.find((option) => /female presenter/i.test(option)) : undefined;
...
const frameExtraction = frameExtractionRequested(cleanInput);
const sourceVideoUrl = materials.find((material) => material.kind === "video" || /^video\//i.test(material.content_type))?.file_url;
const frameTimestampSeconds = Number(cleanInput.match(/(?:at|timestamp|time|saniye|sn)\s*[:=]?\s*(\d+(?:\.\d+)?)/i)?.[1] ?? 0) || undefined;
