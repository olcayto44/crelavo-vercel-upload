  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      {visibleRows.map((item) => {
        ...
        const droneDetails = item.production_type === "drone_video" ? item.request_metadata?.droneDetails as { locationAddress?: string; routePath?: string; markedArea?: string; shotType?: string; mapStyle?: string; cameraMovement?: string; cameraAngle?: string; visualStyle?: string; narrationLanguage?: string; subtitleOption?: string; musicStyle?: string; materialGroups?: Record<string, string[]> } | undefined : undefined;
        const droneDetailsSummary = droneDetails ? [droneDetails.locationAddress, droneDetails.cameraAngle, droneDetails.shotType, droneDetails.mapStyle, droneDetails.visualStyle].filter(Boolean).join(" · ") : "";
          ...
          {droneDetails ? (
            <section className="workspace-action-note" style={{ marginTop: 12 }}>
              ...
              <div className="admin-info-grid" style={{ marginTop: 10 }}>
                ...
                <div><span>Camera angle / view</span><strong>{droneDetails.cameraAngle || "No camera angle"}</strong><small>{droneDetails.locationAddress?.includes("—") ? "Coordinates included" : "Address only"}</small></div>
