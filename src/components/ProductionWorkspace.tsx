...
const [notice, setNotice] = useState("");
  ...
  const rawPreviewUrl = safeAssetUrl(production.preview_url || outputJson.finalVideoUrl || outputJson.previewUrl || outputJson.preview_url);
  const rawDeliveryUrl = safeAssetUrl(production.delivery_zip_url || production.delivery_link || outputJson.finalVideoUrl || outputJson.deliveryZipUrl || outputJson.delivery_url);
  const previewUrl = isMediaProduction && !/final_video_ready|provider_succeeded|completed/i.test(`${production.generation_status ?? ""} ${String(outputJson.providerStatus ?? "")} ${String(outputJson.finalVideoUrl ?? "")}`) ? "" : rawPreviewUrl;
  const deliveryUrl = isMediaProduction && !/final_video_ready|provider_succeeded|completed/i.test(`${production.generation_status ?? ""} ${String(outputJson.providerStatus ?? "")} ${String(outputJson.finalVideoUrl ?? "")}`) ? "" : rawDeliveryUrl;
  ...
  const mediaFinalReady = !isMediaProduction || /final_video_ready|provider_succeeded|completed/i.test(`${production.generation_status ?? ""} ${String(outputJson.providerStatus ?? "")} ${String(outputJson.finalVideoUrl ?? "")}`);
    ...
    <div className="production-workspace-shell">
      <section className="production-workspace-main">
        <div className="production-workspace-head">
          ...
          <div className="production-top-credit-pill" title="Production credit reserve">
            <span>{production.reserved_credits ? "Reserved" : production.estimated_credits ? "Estimate" : "Credits"}</span>
        ...
        <section className={`customer-preview-theater preview-mode-${previewMode}`}>
          <div className="customer-preview-toolbar">
            ...
            <div className="customer-preview-top-actions" aria-label="Preview delivery actions">
              {previewUrl ? <a className="btn" href={previewUrl} target="_blank"><PlayCircle size={14} /> Preview</a> : <button className="btn" type="button" disabled><PlayCircle size={14} /> Preview</button>}
              {deliveryUrl ? <a className="btn secondary" href={deliveryUrl} target="_blank"><Download size={14} /> ZIP</a> : <button className="btn secondary" type="button" disabled><Download size={14} /> ZIP</button>}
              ...
              <button className="btn secondary" type="button" onClick={restartProviderJob}>{isProjectProduction ? "Package" : "Provider"}</button>
            </div>
          </div>
